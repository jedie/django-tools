...
class DynamicSiteTest(BaseTestCase, TestCase):
    def setUp(self):
        ...
        SiteAlias.objects.create(site=self.site1, alias="domain_one_alias_one", regex=False)
        SiteAlias.objects.create(site=self.site1, alias="domain_one_alias_two", regex=False)
        SiteAlias.objects.create(site=self.site2, alias="www.domain_one.tld", regex=False)
        SiteAlias.objects.create(site=self.site2, alias=r"^(.*?\.domain_two\.tld)$", regex=True)
...
if __name__ == "__main__":
    ...
    log.logging.basicConfig(format='LOG: %(message)s')
