...
from django_tools_project.django_tools_test_app.views import delay_view
...
class CacheDelayTests(SimpleTestCase):
    ...
    def test_delay(self):
        ...
        with self.assertLogs(logger="django_tools.debug.delay", level=logging.DEBUG) as log:
            CacheDelay(key="delay", only_debug=False).load(
                request,
                query_string="delay",
            )
        ...
        with (
            self.assertLogs(logger='django_tools.debug.delay', level=logging.DEBUG) as log,
            mock.patch.object(time, 'sleep', return_value=None) as mock_method,
        ):
...
class SessionDelayTests(TestUserMixin, HtmlAssertionMixin, TestCase):
    def setUp(self):
        super().setUp()
        self.client.logout()
    ...
    def test_delay(self):
        ...
        self.assertEqual(response.resolver_match.func, delay_view, response)
        self.assertEqual(response.content, b'django_tools_project.django_tools_test_app.views.delay_view')
    ...
    def test_not_set(self):
        ...
        self.assertEqual(log.output, ["DEBUG:django_tools.debug.delay:No delay for 'delay_view' from session"])
    ...
    def test_message(self):
        ...
        self.assertEqual(
            ...
            [
                "INFO:django_tools.debug.delay:Add 'sec' value to session",
                ("WARNING:django_tools.debug.delay:Save 3 sec. from 'sec' for 'delay_view' into session"),
                "WARNING:django_tools.debug.delay:Delay 3 sec. for 'delay_view'",
            ],
        )

        mock_method.assert_called_once_with(3)
        self.assert_messages(response, expected_messages=["Use 3 sec. 'sec'"])
