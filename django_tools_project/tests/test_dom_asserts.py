...
class DOMassertTest(BaseTestCase):
    ...
    def test_assertDOM_must_contain(self):
        self.assertRaises(
            ...
            self.assertDOM,
            response=FAKE_RESPONSE1,
            must_contain=('<input name="last_name" type="text" value="Bar" id="error" />',),
    ...
    def test_assertDOM_must_not_contain(self):
        self.assertRaises(
            ...
            self.assertDOM,
            response=FAKE_RESPONSE1,
            must_not_contain=('<input name="last_name" type="text" value="Bar" />',),
            use_browser_traceback=False,
        )

    def test_assertContains1(self):
        self.assertContains(
            response=FAKE_RESPONSE1, text='<input name="last_name" type="text" value="Bar" />', count=1, html=True
        )
